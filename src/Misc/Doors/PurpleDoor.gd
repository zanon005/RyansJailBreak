...
func openDoor():
	if(currentState != STATE.OPEN):	
		...
		sprite.texture = load("res://Sprites/Misc/porta-roxa-aberta.png")
...
func closeDoor():
	if(currentState != STATE.CLOSED):
		currentState = STATE.CLOSED
		sprite.texture = load("res://Sprites/Misc/porta-roxa.png")
...
func _on_DoorColisionArea_body_entered(body):
	if(body.is_in_group("Player")):
		...
		if(PlayerInventory.checkIfHasDoorKey("PurpleKey")):
			openDoor()
